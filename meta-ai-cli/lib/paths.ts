import { basename, dirname, extname, join, resolve } from "jsr:@std/path@^1.1.2";
...
export async function planImageOutputs(outputPath: string, count: number, extension = ".jpg"): Promise<string[]> {
  let base = resolvePath(outputPath);
  let directory = /[\\\\/]$/.test(outputPath);
  try { directory ||= (await Deno.stat(base)).isDirectory; }
  catch (error) { if (!(error instanceof Deno.errors.NotFound)) throw error; }
  if (directory) {
    await Deno.mkdir(base, { recursive: true });
    base = join(base, "image");
  }
  const suppliedExtension = extname(base);
  if (suppliedExtension) base = base.slice(0, -suppliedExtension.length);
  return await planNumberedOutputs(base + extension, count, extension);
}
