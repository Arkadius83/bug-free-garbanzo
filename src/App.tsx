...
import { EventStudio } from "./features/events/EventStudio";
...
type AppView = "overview" | "releases" | "events" | "ai-studio" | "calendar" | "analytics" | "publishing" | "contacts" | "settings";
...
export function App() {
  ...
  const [eventCampaignItems,setEventCampaignItems]=useState<EventCampaignItem[]>([]);
  ...
  useEffect(() => {
    if (activeView !== "calendar"||!window.studio)return;const extras=activeReleaseId?Promise.all([window.studio.listCampaignPackItems(activeReleaseId),window.studio.listMediaGenerations(activeReleaseId)]):Promise.resolve([[],[]] as [CampaignPackItem[],MediaGenerationSummary[]]);void Promise.all([window.studio.listTasks(),window.studio.listPublishingQueue(),extras,window.studio.getMetaConnection(),window.studio.listEventCampaignItems()]).then(([savedTasks,queue,[pack,media],metaConnection,eventItems])=>{setTasks(savedTasks);setPublishingQueue(queue);setCampaignPackItems(pack);setMediaGenerations(media);setMeta(metaConnection);setEventCampaignItems(eventItems);}).catch((error) => setTaskMessage(error instanceof Error ? error.message : "Could not load tasks and publishing queue"));
  }, [activeView,activeReleaseId]);
  ...
  return (
    <div className="shell">
      ...
      <main className="app-main">
        ...
        {activeView==="analytics"&&<div className="page-content v4-page analytics-page">
          <PageHeader eyebrow="Analytics Dashboard" title="Catalog intelligence." lead="Real SoundCloud snapshots, Spotify catalog data and local campaign progress." actions={<><div className="analytics-filters"><label>Artist alias<select value={analyticsArtist} onChange={(event)=>setAnalyticsArtist(event.target.value as ArtistAlias|"all")}><option value="all">All aliases</option>{artists.map((profile)=><option value={profile.id} key={profile.id}>{profile.name}</option>)}</select></label><label>Trend window<select value={analyticsPeriod} onChange={(event)=>setAnalyticsPeriod(Number(event.target.value) as 7|30|90)}><option value={7}>Last 7 days</option><option value={30}>Last 30 days</option><option value={90}>Last 90 days</option></select></label></div></>} />
          {analyticsMessage&&<p className="analytics-note">{analyticsMessage}</p>}
          <div className="analytics-kpis"><SectionCard title="SoundCloud plays"><article><span>SoundCloud plays</span><b>{analyticsTotals.plays.toLocaleString()}</b><em>{analyticsTotals.tracked?`${analyticsTotals.playsDelta>=0?"+":""}${analyticsTotals.playsDelta.toLocaleString()} in ${analyticsPeriod}d`:"collecting snapshots"}</em></article></SectionCard><SectionCard title="Interactions"><article><span>Interactions</span><b>{(analyticsTotals.likes+analyticsTotals.comments+analyticsTotals.reposts).toLocaleString()}</b><small>{analyticsTotals.likes.toLocaleString()} likes</small></article></SectionCard><SectionCard title="Imported tracks"><article><span>Imported tracks</span><b>{analyticsTracks.length}</b><small>{analyticsTracks.filter((track)=>track.artistId).length} assigned</small></article></SectionCard><SectionCard title="Spotify releases"><article><span>Spotify releases</span><b>{analyticsSpotify.length}</b><small>catalog entries only</small></article></SectionCard><SectionCard title="Release readiness"><article><span>Release readiness</span><b>{averageReadiness}%</b><small>{analyticsReleases.length} local releases</small></article></SectionCard><SectionCard title="Campaign posts"><article><span>Campaign posts</span><b>{analyticsQueue.length}</b><small>{analyticsQueue.filter((item)=>item.status==="scheduled").length} scheduled · {analyticsQueue.filter((item)=>item.status==="published").length} published</small></article></SectionCard></div>
          <div className="analytics-grid">
            <SectionCard eyebrow="Performance ranking" title="Tracks worth your attention" className="analytics-panel"><span className="eyebrow">Performance ranking</span><h2>Tracks worth your attention</h2>{analyticsRanked.length?<div className="analytics-ranking">{analyticsRanked.map((track,index)=>{const window=analyticsPerformance[track.id]?.windows.find((item)=>item.days===analyticsPeriod);return <article className="v4-card" key={track.id}><span>{index+1}</span><div><strong>{track.title}</strong><small>{artists.find((profile)=>profile.id===track.artistId)?.name??"Unassigned"} · {track.contentType} · {track.catalogStatus}</small></div><b>{(track.playbackCount??0).toLocaleString()}<small>plays</small></b><b>{track.engagementRate===null?"—":`${track.engagementRate}%`}<small>engagement</small></b><b>{window?.available?`${(window.playsDelta??0)>=0?"+":""}${window.playsDelta}`:"—"}<small>{analyticsPeriod}d plays</small></b></article>})}</div>:<div className="analytics-empty">No SoundCloud tracks match this alias.</div>}</SectionCard>
            <SectionCard eyebrow="Catalog structure" title="Rights and classification" className="analytics-panel"><span className="eyebrow">Catalog structure</span><h2>Rights and classification</h2><div className="analytics-bars">{analyticsContentTypes.map((item)=><div key={item.type}><div className="analytics-bar-head"><span>{item.type.replace("official-remix","official remix")}</span><b>{item.count}</b></div><div className="analytics-bar-track"><i style={{width:`${analyticsTracks.length?item.count/analyticsTracks.length*100:0}%`}}/></div></div>)}</div><h2>Editorial status</h2><div className="analytics-bars">{analyticsCatalogStatuses.map((item)=><div key={item.status}><div className="analytics-bar-head"><span>{item.status}</span><b>{item.count}</b></div><div className="analytics-bar-track"><i style={{width:`${analyticsTracks.length?item.count/analyticsTracks.length*100:0}%`}}/></div></div>)}</div></SectionCard>
            <SectionCard eyebrow="Release operations" title="Readiness and next blockers" className="analytics-panel analytics-readiness"><span className="eyebrow">Release operations</span><h2>Readiness and next blockers</h2>{analyticsReleases.length?<div className="analytics-readiness-list">{analyticsReleases.map((release)=>{const readiness=analyticsReadiness[release.id];const score=readiness?.score??0;return <article key={release.id} onClick={()=>openReleaseWorkspace(release)}><div><strong>{release.title}</strong><span>{score}%</span></div><p>{release.artistName} · {release.status} · {readiness?.missing[0]??"Ready for campaign"}</p><div className="analytics-progress"><i style={{width:`${score}%`}}/></div></article>})}</div>:<div className="analytics-empty">No local releases for this alias.</div>}</SectionCard>
          </div>
