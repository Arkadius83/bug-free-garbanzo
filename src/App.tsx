...
import type { AiSettings, AssetKind, AssetSummary, AudioAnalysisSummary, ArtistAlias, CampaignPackItem, CatalogMatchSuggestion, DatabaseHealth, DraftStatus, DraftSummary, GeneratedCampaignDraft, MediaGenerationSettings, MediaGenerationSummary, MediaProvider, ReleaseReadiness, ReleaseStatus, ReleaseSummary, SoundCloudCatalogStatus, SoundCloudConnection, SoundCloudContentType, SoundCloudTrackPerformance, SoundCloudTrackSummary, SpotifyConnection, SpotifyReleaseSummary, SystemStatus, TaskAssignee, TaskPriority, TaskStatus, TaskSummary } from "../electron/shared/contracts";
...
export function App() {
  ...
  const [mediaSettings,setMediaSettings]=useState<MediaGenerationSettings>({openAiConfigured:false,klingConfigured:false});
  ...
  useEffect(() => {
    ...
    void Promise.all([window.studio.getSoundCloudConnection(), window.studio.listSoundCloudTracks(), window.studio.getSpotifyConnection(), window.studio.getSpotifyArtistMappings(), window.studio.listSpotifyReleases(), window.studio.getCatalogMatchSuggestions(),window.studio.getMediaGenerationSettings()]).then(([connection, tracks, spotifyConnection, mappings, savedSpotifyReleases, matches,media]) => { setSoundCloud(connection); setSoundCloudTracks(tracks); setSpotify(spotifyConnection); setSpotifyArtistIds((current) => ({ ...current, ...Object.fromEntries(mappings.map((mapping) => [mapping.artistId, mapping.spotifyArtistId])) })); setSpotifyReleases(savedSpotifyReleases); setCatalogMatches(matches);setMediaSettings(media); }).catch((error) => setSoundCloudMessage(error instanceof Error ? error.message : "Could not load integrations"));
  }, [activeView]);
  ...
  async function generateMedia(item:CampaignPackItem,provider:MediaProvider,mediaType:"image"|"video"){if(!window.studio)return;setMediaBusy(item.id);setMediaMessage(`Starting ${provider} ${mediaType} generation. This may use paid credits...`);try{const result=await window.studio.generateMedia({campaignPackItemId:item.id,provider,mediaType});setMediaGenerations((current)=>[result,...current.filter((row)=>row.id!==result.id)]);setMediaMessage(result.status==="generating"?"Kling accepted the task. Use Refresh in the gallery after generation finishes.":"Generated media downloaded to the local gallery.");}catch(error){setMediaMessage(error instanceof Error?error.message.replace(/^Error invoking remote method '[^']+': Error: /,""):"Media generation failed");}finally{setMediaBusy(null);}}
  async function refreshMedia(id:string){if(!window.studio)return;setMediaBusy(id);const updated=await window.studio.refreshMediaGeneration(id);setMediaGenerations((current)=>current.map((row)=>row.id===id?updated:row));setMediaBusy(null);}
