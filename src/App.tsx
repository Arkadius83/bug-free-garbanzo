...
import type { AiSettings, AssetKind, AssetSummary, AudioAnalysisSummary, ArtistAlias, DatabaseHealth, DraftStatus, DraftSummary, GeneratedCampaignDraft, ReleaseReadiness, ReleaseStatus, ReleaseSummary, SoundCloudCatalogStatus, SoundCloudConnection, SoundCloudContentType, SoundCloudTrackPerformance, SoundCloudTrackSummary, SpotifyConnection, SpotifyReleaseSummary, SystemStatus, TaskAssignee, TaskPriority, TaskStatus, TaskSummary } from "../electron/shared/contracts";
...
export function App() {
  ...
  useEffect(() => {
    ...
    void Promise.all([window.studio.getSoundCloudConnection(), window.studio.listSoundCloudTracks(), window.studio.getSpotifyConnection(), window.studio.getSpotifyArtistMappings(), window.studio.listSpotifyReleases()]).then(([connection, tracks, spotifyConnection, mappings, savedSpotifyReleases]) => { setSoundCloud(connection); setSoundCloudTracks(tracks); setSpotify(spotifyConnection); setSpotifyArtistIds((current) => ({ ...current, ...Object.fromEntries(mappings.map((mapping) => [mapping.artistId, mapping.spotifyArtistId])) })); setSpotifyReleases(savedSpotifyReleases); }).catch((error) => setSoundCloudMessage(error instanceof Error ? error.message : "Could not load integrations"));
  }, [activeView]);
  ...
  async function syncSpotifyCatalog() { if (!window.studio) return; setSpotifyBusy(true); try { const items = await window.studio.syncSpotifyCatalog(); setSpotifyReleases(items); setSpotifyMessage(`Spotify catalog synchronized: ${items.length} releases.`); } catch (error) { setSpotifyMessage(error instanceof Error ? error.message.replace(/^Error invoking remote method '[^']+': Error: /, "") : "Spotify sync failed"); } finally { setSpotifyBusy(false); } }
