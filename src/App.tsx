...
export function App() {
  ...
  async function generateMedia(item:CampaignPackItem,provider:MediaProvider,mediaType:"image"|"video"){if(!window.studio)return;setMediaBusy(item.id);setMediaMessage(provider==="comfyui"?"Sending brand-enhanced image prompt to local ComfyUI...":`Starting ${provider} ${mediaType} generation. This may use paid credits...`);try{const result=await window.studio.generateMedia({campaignPackItemId:item.id,provider,mediaType,...(imageAspect==="default"?{}:{aspectRatio:imageAspect})});setMediaGenerations((current)=>[result,...current.filter((row)=>row.id!==result.id)]);setMediaMessage(result.status==="generating"?`${provider==="comfyui"?"ComfyUI":"Kling"} accepted the task. Use Refresh in the gallery after it finishes.`:"Generated media downloaded to the local gallery.");}catch(error){setMediaMessage(error instanceof Error?error.message.replace(/^Error invoking remote method '[^']+': Error: /,""):"Media generation failed");}finally{setMediaBusy(null);}}
