...
export function App() {
  ...
  const [meta,setMeta]=useState<MetaConnection|null>(null);const [metaAppId,setMetaAppId]=useState("");const [metaAppSecret,setMetaAppSecret]=useState("");const [metaBusy,setMetaBusy]=useState(false);const [metaMessage,setMetaMessage]=useState("");const [metaDestinationByItem,setMetaDestinationByItem]=useState<Record<string,string>>({});const [metaQueueItemId,setMetaQueueItemId]=useState("");
  ...
  async function saveMetaCredentials(){if(!window.studio)return;setMetaBusy(true);try{setMeta(await window.studio.saveMetaCredentials(metaAppId,metaAppSecret));setMetaAppSecret("");setMetaMessage("Meta credentials saved with Windows encryption.");}catch(error){setMetaMessage(error instanceof Error?error.message:"Could not save Meta credentials");}finally{setMetaBusy(false);}}
