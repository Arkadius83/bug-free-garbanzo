...
import type { AiSettings, AssetKind, AssetSummary, AudioAnalysisSummary, ArtistAlias, BrandProfile, CampaignChannel, CampaignPackItem, CatalogMatchSuggestion, ContactChannel, ContactRelationshipStatus, ContactSummary, ContactType, DatabaseHealth, DraftStatus, DraftSummary, GeneratedCampaignDraft, LocalServiceStatus, MediaAspectRatio, MediaBridgeStatus, MediaGenerationSettings, MediaGenerationSummary, MediaProvider, MetaConnection, PublishingQueueItem, ReleaseReadiness, ReleaseStatus, ReleaseSummary, ScheduleEvent, SoundCloudCatalogStatus, SoundCloudConnection, SoundCloudContentType, SoundCloudTrackPerformance, SoundCloudTrackSummary, SpotifyConnection, SpotifyReleaseSummary, SystemStatus, TaskAssignee, TaskPriority, TaskStatus, TaskSummary, UpsertContactInput } from "../electron/shared/contracts";
...
type AppView = "overview" | "releases" | "ai-studio" | "calendar" | "analytics" | "publish-analytics" | "contacts" | "integrations" | "settings";
...
const navigation: Array<{ id: AppView | "placeholder"; label: string; icon: string }> = [
  ...
  { id: "publish-analytics", label: "Publish Analytics", icon: "↗" },
...
export function App() {
  ...
  useEffect(() => {
    ...
    void Promise.all([window.studio.listTasks(), window.studio.listPublishingQueue(), window.studio.listScheduleEvents(), window.studio.getMetaConnection(), window.studio.getSoundCloudConnection(), window.studio.getSpotifyConnection()]).then(([savedTasks, queue, events, metaConnection, soundCloudConnection, spotifyConnection]) => {
      setTasks(savedTasks); setPublishingQueue(queue); setMeta(metaConnection); setSoundCloud(soundCloudConnection); setSpotify(spotifyConnection);
    ...
    <div className="shell">
      ...
      <main className="app-main">
        ...
        {activeView === "overview" && <Dashboard
          ...
          queue={publishingQueue} events={dashboardEvents} meta={meta} soundCloud={soundCloud} spotify={spotify} system={status} database={database}
        ...
        {activeView==="publish-analytics"&&<div className="page-content"><PostPublishAnalytics publishingQueue={publishingQueue} /></div>}
...
{activeView==="settings"&&<section className="page-content developer-mode-page"><div className="panel developer-mode-gate"><div><span className="eyebrow">Settings &gt; Developer Mode</span><h2>Execution Monitor</h2><p>Developer diagnostics are gated here so normal AI Studio work stays conversational.</p></div><label className="developer-toggle"><input type="checkbox" checked={developerModeEnabled} onChange={(event)=>setDeveloperModeEnabled(event.target.checked)}/> Developer Mode enabled</label></div>{developerModeEnabled&&<HarnessPlanPreview release={currentRelease} artistId={selectedArtist} artistName={artist.name} defaultInstruction={`Create a plan-only promotional workflow for ${currentRelease?.title ?? (title || "the active release")}: generate cover concepts, analyze artwork readiness, upscale final assets, transform files for social channels and draft campaign copy.`}/>}</section>}
        ...
        {activeView === "releases" && <div className="page-content release-page release-page-v3">
        <header>
