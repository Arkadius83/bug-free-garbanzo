...
export function ReleaseReadinessCard({ readiness }: { readiness: ReleaseReadiness | null }) {
  ...
  return <SurfacePanel className="foundation-card readiness-card"><h2>Release Readiness</h2><div className="readiness-summary"><svg className="readiness-ring" viewBox="0 0 100 100" role="img" aria-label={score === null ? "Readiness not available" : `${score}% release readiness`}><circle cx="50" cy="50" r="42" className="ring-track" /><circle cx="50" cy="50" r="42" className="ring-value" pathLength="100" strokeDasharray={`${score ?? 0} 100`} /><text x="50" y="55" textAnchor="middle">{score === null ? "--" : `${score}%`}</text></svg><p>{readiness?.missing[0] ?? (readiness ? "All readiness checks passed" : "Save a release to check readiness")}</p></div><ul className="readiness-checks">{readiness?.checks.map(check => <li key={check.id}><span className={check.complete ? "check-complete" : "check-pending"} aria-hidden="true">{check.complete ? "✓" : "○"}</span><span>{check.label}</span><span className="sr-only">{check.complete ? "Complete" : "Pending"}</span></li>)}</ul></SurfacePanel>;
}
  ...
  return <section className="release-foundation-v31">
    ...
    <div className="foundation-grid">
      <div className="foundation-column">
        <SurfacePanel className="foundation-card" data-save-error={props.saveError || undefined}><header><h2>Basic Information</h2><StatusBadge label={props.releaseId ? "Saved release" : "New release"} tone="cyan" /></header>{metadata}<div className="foundation-save"><Button onClick={props.onSave}>{saved ? "Saved ✓" : props.releaseId ? "Save changes" : "Create release"}</Button><p role={props.saveError ? "alert" : "status"}>{props.saveMessage}</p></div></SurfacePanel>
        <SurfacePanel className="foundation-card" data-uploaded={uploaded || undefined} data-analyzing={props.analyzing || undefined}><header><h2>Audio & Tracks</h2><Button variant="ghost" onClick={() => props.onAttach("audio")}>Choose audio</Button></header>{props.analyzing && <div className="analysis-wave" role="status" aria-label="Analyzing audio">{Array.from({ length: 16 }, (_, index) => <i key={index} style={{ animationDelay: `${index * 45}ms` }} />)}</div>}{audio.length ? <><p>{audio.length} attached audio file{audio.length === 1 ? "" : "s"}</p><div className="foundation-file-names">{audio.map(asset => <strong key={asset.id}>{asset.fileName}</strong>)}</div><Button variant="secondary" onClick={() => setDrawer("audio")}>Tracks & audio analysis</Button></> : <p className="foundation-empty">No audio attached.</p>}<p className="foundation-feedback" role="status">{props.assetMessage}</p></SurfacePanel>
        <SurfacePanel className="foundation-card"><details className="foundation-disclosure" open><summary>Lyrics & Credits</summary><div><dl className="foundation-metadata"><div><dt>Artist</dt><dd>{props.artist}</dd></div></dl><p className="foundation-empty">Dedicated lyrics and contributor fields are not available for this release.</p></div></details></SurfacePanel>
      </div>
      <div className="foundation-column">
        <SurfacePanel className="foundation-card"><header><h2>Metadata & Identifiers</h2><Button variant="ghost" onClick={() => setDrawer("metadata")}>Edit</Button></header><dl className="foundation-metadata"><div><dt>Release ID</dt><dd>{props.releaseId || "Not saved"}</dd></div><div><dt>Genre</dt><dd>{props.genre || "Not set"}</dd></div><div><dt>ISRC / UPC</dt><dd>Not available</dd></div></dl></SurfacePanel>
        <SurfacePanel className="foundation-card" data-uploaded={uploaded || undefined}><header><h2>Artwork</h2><Button variant="ghost" onClick={() => setDrawer("artwork")}>Manage</Button></header><div className="foundation-art-summary"><MediaSurface selected={Boolean(cover)} emptyLabel="No artwork">{cover ? <img src={cover} alt={`${props.title} cover`} /> : undefined}</MediaSurface><div><strong>{coverAsset?.fileName || "Attach release artwork"}</strong><p>{coverAsset?.width && coverAsset.height ? `${coverAsset.width} × ${coverAsset.height} px` : "Square cover recommended"}</p><small>Original file stays in its folder.</small></div></div></SurfacePanel>
        <SurfacePanel className="foundation-card"><header><h2>Notes & References</h2><Button variant="ghost" onClick={() => setDrawer("notes")}>Expand</Button></header>{notes}</SurfacePanel>
      </div>
      <aside className="foundation-column foundation-sidebar">
        <ReleaseReadinessCard readiness={readiness} />
        <SurfacePanel className="foundation-card"><header><h2>Key Dates</h2><Button variant="ghost" onClick={() => setDrawer("dates")}>Edit</Button></header>{dates}</SurfacePanel>
        <SurfacePanel className="foundation-card foundation-actions"><h2>Quick Actions</h2><Button variant="secondary" onClick={() => props.onNavigate("campaign-drafts")}>Campaign drafts →</Button><Button variant="secondary" onClick={() => props.onNavigate("release-plan")}>Release plan →</Button><Button variant="ghost" onClick={() => props.onNavigate("promotion-formats")}>Promotion formats →</Button></SurfacePanel>
      </aside>
    </div>
