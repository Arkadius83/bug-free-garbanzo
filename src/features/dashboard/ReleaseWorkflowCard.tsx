...
export function ReleaseWorkflowCard({ release, readiness, tasks, onOpenTasks }: ReleaseWorkflowCardProps) {
  ...
  if (!release) return <SurfacePanel variant="standard" className="dashboard-panel release-workflow-card"><div className="dashboard-panel-heading"><span className="dashboard-eyebrow">Release workflow</span><StatusBadge label="No release" /></div><p className="workflow-empty">Select or create a release to see its workflow state.</p></SurfacePanel>;
  ...
  return <SurfacePanel variant="standard" className="dashboard-panel release-workflow-card">
    <div className="dashboard-panel-heading"><span className="dashboard-eyebrow">Release workflow</span><StatusBadge label={release.status} tone="cyan" /></div>
    <h2>{release.title}</h2>
    ...
    <div className="workflow-details"><span>{openTasks} open task{openTasks === 1 ? "" : "s"}</span><span>{readiness?.missing[0] ?? "No readiness blocker"}</span></div>
