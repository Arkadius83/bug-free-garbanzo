import type { DatabaseHealth, MetaConnection, PublishingQueueItem, ScheduleEvent, SoundCloudConnection, SpotifyConnection, SystemStatus } from "../../../electron/shared/contracts";
import { StatusBadge, type StatusBadgeTone } from "../../ui/StatusBadge";
import { SurfacePanel } from "../../ui/SurfacePanel";

type Props = { queue?: PublishingQueueItem[]; events?: ScheduleEvent[]; meta?: MetaConnection | null; soundCloud?: SoundCloudConnection | null; spotify?: SpotifyConnection | null; system?: SystemStatus | null; database?: DatabaseHealth | null; readinessMissing?: string[]; };
...
export function DashboardOperations({ queue = [], events = [], meta = null, soundCloud = null, spotify = null, system = null, database = null, readinessMissing = [] }: Props) {
  ...
  const platforms = [
    ["Spotify", spotify?.connected ?? null], ["SoundCloud", soundCloud?.connected ?? null], ["Instagram", meta?.destinations.some((item) => item.platform === "Instagram") ?? null], ["Facebook", meta?.destinations.some((item) => item.platform === "Facebook") ?? null], ["YouTube", null], ["TikTok", null], ["DistroKid", null]
  ] as const;
  ...
  return <>
    <section className="dashboard-operations-grid">
      <SurfacePanel className="dashboard-operation-panel platform-performance">
        ...
        <div className="platform-status-list">{platforms.map(([name, connected]) => <div key={name}><strong>{name}</strong><StatusBadge label={label(connected)} tone={tone(connected)} /></div>)}</div>
      </SurfacePanel>
      <SurfacePanel className="dashboard-operation-panel campaign-performance">
