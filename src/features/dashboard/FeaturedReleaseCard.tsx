...
interface FeaturedReleaseCardProps { release: ReleaseSummary | undefined; coverAsset: AssetSummary | undefined; coverUrl?: string; readiness: ReleaseReadiness | null; onOpenRelease: () => void; audioSource?: string; isPlaying: boolean; onPlay: () => void; }
...
export function FeaturedReleaseCard({ release, coverAsset, coverUrl, readiness, onOpenRelease, audioSource, isPlaying, onPlay }: FeaturedReleaseCardProps) {
  ...
  return <SurfacePanel variant="highlight" state="active" className="dashboard-panel featured-release-card">
    ...
    <div className="featured-release-body">
      <div className="featured-release-art">{showCover ? <img src={coverUrl!} alt={`${release.title} cover artwork`} onError={() => setFailedCoverUrl(coverUrl)} /> : <span aria-label="Cover artwork unavailable">{initials(release.title)}</span>}</div>
    ...
    <div className="featured-release-footer">
      <div className="featured-release-footer-left">
        <StatusBadge label={titleCase(release.status)} tone={releaseTone(release.status)} />
        <button
          className="featured-release-play-btn"
