...
const api: StudioApi = {
  ...
  verifyPublishedPost: (itemId: string) => ipcRenderer.invoke("studio:verify-published-post", itemId).catch(() => Promise.reject(new Error("Post verification not consolidated â€” UI disabled"))),
  fetchAndStorePostAnalytics: (itemId: string) => ipcRenderer.invoke("studio:fetch-post-analytics", itemId).catch(() => Promise.reject(new Error("Analytics not consolidated â€” UI disabled"))),
