...
export function buildProviderTraceViewModel(trace: ProviderExecutionTrace | null | undefined): ProviderTraceViewModel {
  if (!trace || !Array.isArray(trace.diagnostics) || trace.diagnostics.length === 0) {
    return { routingDecision: "", visible: false, finalStatus: "", statusTone: "neutral", duration: "unknown", fallbackUsed: "No", attempts: [] };
  }

  return {
    routingDecision: trace.selectedProvider ? `Auto selected ${trace.selectedProvider} / ${trace.selectedModel ?? "unknown model"}` : "Auto routing: no selected provider",
    visible: true,
    ...
    attempts: trace.diagnostics.map((diagnostic) => ({
      model: diagnostic.model ?? "not reported",
