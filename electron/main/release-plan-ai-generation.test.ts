...
test("validateAndNormalizeDraft rejects mixed valid and invalid items", () => {
  const draft = JSON.parse(VALID_AI_RESPONSE);
  draft.items[1].contentType = "invalid";
  assert.equal(validateAndNormalizeDraft(draft), null);
});
...
async function withGenerationDatabase(run: (database: StudioDatabase, releaseId: string, file: string) => Promise<void>) {
  const directory = mkdtempSync(path.join(os.tmpdir(), "release-plan-ai-service-"));
  const file = path.join(directory, "studio.sqlite");
  const database = new StudioDatabase(file);
  try {
    database.initialize();
    const release = database.createReleaseDraft({ artistId: "arkadelic", title: "AI Service", primaryGenre: "Psytrance", story: "Test context", releaseDate: "2026-10-15" });
    await run(database, release.id, file);
  } finally { database.close(); rmSync(directory, { recursive: true, force: true }); }
}

test("service assembles context, uses injected router, persists and survives reopen", async () => withGenerationDatabase(async (database, releaseId, file) => {
  let prompt = "";
  database.attachAsset({ releaseId, kind: "cover", filePath: "/private/never-sent.png", fileName: "cover.png", mimeType: "image/png", sizeBytes: 1, width: 3000, height: 3000, modifiedAt: null });
  const router: ConversationProviderRouter = async (value, options) => {
    prompt = value;
    assert.ok(options?.signal);
    return createMockProviderRouter(VALID_AI_RESPONSE)(value, options);
  };
  const plan = await generateAndPersistAiReleasePlan(database, router, { releaseId, actor: "tester" });
  assert.match(prompt, /AI Service/);
  assert.match(prompt, /Cover artwork: attached/);
  assert.match(prompt, /Brand direction/);
  assert.match(prompt, /"items"/);
  assert.ok(!prompt.includes("campaignItems"));
  assert.ok(!prompt.includes("/private/"));
  assert.equal(plan.createdBy, "tester");
...
for (const [label, output] of [
  ...
  ["invalid mixed domain items", JSON.stringify({ ...JSON.parse(VALID_AI_RESPONSE), items: [...JSON.parse(VALID_AI_RESPONSE).items, { title: "Bad", purpose: "", contentType: "invalid", targetPlatforms: ["Facebook"] }] })],
  ["impossible date", VALID_AI_RESPONSE.replace("2026-10-15", "2026-02-30")],
  ["unknown platform in a valid list", VALID_AI_RESPONSE.replace('"Facebook"', '"__proto__"')]
]) {
  test("service rejects " + label + " without partial writes", async () => withGenerationDatabase(async (database, releaseId, file) => {
    const original = database.generateReleasePlan({ releaseId });
    const raw = new DatabaseSync(file);
    try {
      const before = raw.prepare("SELECT COUNT(*) AS count FROM events").get()?.count;
      await assert.rejects(generateAndPersistAiReleasePlan(database, createMockProviderRouter(output), { releaseId }), /No plan was saved/);
      assert.deepEqual(database.listReleasePlans(releaseId), [original]);
      assert.equal(raw.prepare("SELECT COUNT(*) AS count FROM events").get()?.count, before);
    } finally { raw.close(); }
  }));
}

test("service provider rejection is sanitized and saves nothing", async () => withGenerationDatabase(async (database, releaseId) => {
  await assert.rejects(generateAndPersistAiReleasePlan(database, async () => { throw new Error("secret-token-must-not-escape"); }, { releaseId }), error => {
    assert.equal((error as Error).message, "Release Plan provider failed. No plan was saved.");
    return true;
  });
  await assert.rejects(generateAndPersistAiReleasePlan(database, createFailingProviderRouter(), { releaseId }), /No plan was saved/);
  assert.deepEqual(database.listReleasePlans(releaseId), []);
}));

test("service cancellation aborts router and prevents persistence", async () => withGenerationDatabase(async (database, releaseId) => {
  const controller = new AbortController();
  const router: ConversationProviderRouter = async (_prompt, options) => {
    controller.abort();
    assert.equal(options?.signal?.aborted, true);
    return createMockProviderRouter(VALID_AI_RESPONSE)("");
  };
  await assert.rejects(generateAndPersistAiReleasePlan(database, router, { releaseId }, false, { signal: controller.signal }), { name: "AbortError" });
  assert.deepEqual(database.listReleasePlans(releaseId), []);
}));

test("service timeout aborts non-cooperative provider and ignores late success", async () => withGenerationDatabase(async (database, releaseId) => {
  let finish!: (result: ConversationProviderRouteResult) => void;
  let signal: AbortSignal | undefined;
  const router: ConversationProviderRouter = (_prompt, options) => {
    signal = options?.signal;
    return new Promise(resolve => { finish = resolve; });
  };
  await assert.rejects(generateAndPersistAiReleasePlan(database, router, { releaseId }, false, { timeoutMs: 0 }), { name: "TimeoutError" });
  assert.equal(signal?.aborted, true);
  finish(await createMockProviderRouter(VALID_AI_RESPONSE)(""));
  await new Promise(resolve => setImmediate(resolve));
  assert.deepEqual(database.listReleasePlans(releaseId), []);
}));

test("service pre-cancellation does not call provider", async () => withGenerationDatabase(async (database, releaseId) => {
  const controller = new AbortController();
  controller.abort();
  await assert.rejects(generateAndPersistAiReleasePlan(database, async () => { assert.fail("must not call"); }, { releaseId }, false, { signal: controller.signal }), { name: "AbortError" });
}));

test("service regeneration includes previous plan and preserves approved revision", async () => withGenerationDatabase(async (database, releaseId) => {
  const first = database.generateReleasePlan({ releaseId });
  database.changeReleasePlanStatus({ id: first.id, status: "REVIEWED" });
  const approved = database.approveReleasePlan({ id: first.id });
  const router: ConversationProviderRouter = async prompt => {
    assert.match(prompt, /Previous plan revision/);
    assert.ok(prompt.includes(first.title));
    return createMockProviderRouter(VALID_AI_RESPONSE)(prompt);
  };
  const next = await generateAndPersistAiReleasePlan(database, router, { releaseId, reason: "New direction" }, true);
  assert.equal(next.revisionNumber, 2);
  assert.equal(next.previousPlanId, first.id);
  assert.equal(next.status, "DRAFT");
  assert.deepEqual(database.getReleasePlan(first.id), approved);
  assert.throws(() => database.updateReleasePlan({ id: first.id, title: "Forbidden" }), /immutable/);
}));

test("router timeout status prevents generation even when a payload exists", async () => {
  const router: ConversationProviderRouter = async () => {
    const result = await createMockProviderRouter(VALID_AI_RESPONSE)("");
    result.trace!.finalStatus = "timeout";
    return result;
  };
  await assert.rejects(generateAiReleasePlanDraft(createTestContext(), { providerRouter: router }), { name: "TimeoutError" });
});

test("oversized output and item counts are rejected", async () => {
  assert.equal(await generateAiReleasePlanDraft(createTestContext(), { providerRouter: createMockProviderRouter(" ".repeat(200001) + VALID_AI_RESPONSE) }), null);
  const draft = JSON.parse(VALID_AI_RESPONSE);
