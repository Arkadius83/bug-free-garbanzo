...
import { DistroKidClient } from "./distrokid.js";
...
import { buildEventCampaign, generateEventCopy } from "./event-generation.js";
...
ipcMain.handle("studio:list-events",()=>studioDatabase.listEvents());
ipcMain.handle("studio:get-event",(_event,id:string)=>studioDatabase.getEvent(id));
ipcMain.handle("studio:save-event",(_event,input:UpsertEventInput)=>studioDatabase.saveEvent(input));
ipcMain.handle("studio:delete-event",(_event,id:string)=>studioDatabase.deleteEvent(id));
ipcMain.handle("studio:list-event-campaign-items",(_event,eventId?:string|null)=>studioDatabase.listEventCampaignItems(eventId));
ipcMain.handle("studio:update-event-campaign-item",(_event,id:string,input:{content?:string;scheduledAt?:string|null;status?:EventCampaignStatus})=>studioDatabase.updateEventCampaignItem(id,input));
ipcMain.handle("studio:enqueue-event-campaign-item",(_event,id:string)=>studioDatabase.enqueueEventCampaignItem(id));
ipcMain.handle("studio:generate-event-content",async(_event,input:GenerateEventContentInput)=>{const event=studioDatabase.getEvent(input.eventId);if(!event)throw new Error("Event not found");const content=await generateEventCopy(event,input,conversationProviderRouter);return studioDatabase.saveEventCampaignItem({eventId:event.id,title:input.contentType.replaceAll("_"," ").toLowerCase(),contentType:input.contentType,language:input.language,platform:input.platform,tone:input.tone,content,scheduledAt:null,status:"REVIEW"});});
ipcMain.handle("studio:generate-event-campaign",(_event,input:GenerateEventCampaignInput)=>{const event=studioDatabase.getEvent(input.eventId);if(!event)throw new Error("Event not found");return buildEventCampaign(event,input).map(item=>studioDatabase.saveEventCampaignItem(item));});
...
let distrokidClient: DistroKidClient;
...
ipcMain.handle("studio:get-distrokid-connection", () => distrokidClient.status());
ipcMain.handle("studio:prepare-distrokid-form", (_event, releaseId: string) => distrokidClient.prepareFormPayload(releaseId));
ipcMain.handle("studio:open-distrokid-upload", (_event, releaseId: string) => distrokidClient.openUpload(releaseId));
ipcMain.handle("studio:fill-distrokid-form", () => distrokidClient.fillForm());
ipcMain.handle("studio:close-distrokid-session", () => distrokidClient.closeSession());
...
ipcMain.handle("studio:get-tiktok-connection", () => tikTokClient.status());
ipcMain.handle("studio:save-tiktok-credentials", (_event, clientKey: string, clientSecret: string) => tikTokClient.saveCredentials(clientKey, clientSecret));
ipcMain.handle("studio:begin-tiktok-connect", () => tikTokClient.beginConnect());
ipcMain.handle("studio:disconnect-tiktok", () => tikTokClient.disconnect());
ipcMain.handle("studio:get-tiktok-creator-info", async (): Promise<TikTokCreatorInfo> => tikTokClient.getCreatorInfo());
ipcMain.handle("studio:get-tiktok-user-profile", async (): Promise<TikTokUserProfile> => tikTokClient.getUserProfile());
ipcMain.handle("studio:select-tiktok-test-video", async (event) => {
  const owner = BrowserWindow.fromWebContents(event.sender) ?? undefined;
  const options = { properties: ["openFile"] as Array<"openFile">, filters: [{ name: "Video", extensions: ["mp4", "mov", "webm"] }] };
  const result = owner ? await dialog.showOpenDialog(owner, options) : await dialog.showOpenDialog(options);
  return result.canceled ? null : result.filePaths[0] ?? null;
});
ipcMain.handle("studio:publish-tiktok-test", async (_event, input: TikTokTestPublishInput): Promise<TikTokTestPublishResult> => {
  console.info("[tiktok-test] request started", { mode: input.mode, startedAt: new Date().toISOString() });
  const result = await tikTokClient.publishTest(input);
  if (result.ok) console.info("[tiktok-test] response received", { publishId: result.publishId, mode: result.mode });
  else console.warn("[tiktok-test] request failed", { error: result.sanitizedError });
  return result;
});
...
ipcMain.handle("studio:select-and-attach-event-asset",async(event,eventId:string,kind:EventAsset["kind"])=>{const owner=BrowserWindow.fromWebContents(event.sender)??undefined;const result=owner?await dialog.showOpenDialog(owner,{properties:["openFile"],filters:[{name:"Event media",extensions:["png","jpg","jpeg","webp","svg","mp4","mov"]}]}):await dialog.showOpenDialog({properties:["openFile"],filters:[{name:"Event media",extensions:["png","jpg","jpeg","webp","svg","mp4","mov"]}]});if(result.canceled||!result.filePaths[0])return null;const filePath=result.filePaths[0];return studioDatabase.attachEventAsset({eventId,kind,filePath,fileName:path.basename(filePath),mimeType:inferMimeType(filePath),metadata:{purpose:kind}});});
ipcMain.handle("studio:detach-event-asset",(_event,id:string)=>studioDatabase.detachEventAsset(id));
...
void app.whenReady().then(async () => {
  ...
  distrokidClient = new DistroKidClient(studioDatabase, app.getPath("userData"));
