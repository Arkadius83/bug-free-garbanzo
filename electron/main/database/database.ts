...
export class StudioDatabase {
  ...
  createPublishingQueueItem(input:CreatePublishingQueueInput):PublishingQueueItem{const caption=this.database.prepare("SELECT release_id AS releaseId,content,status,kind FROM campaign_pack_items WHERE id=?").get(input.campaignPackItemId) as {releaseId:string;content:string;status:DraftStatus;kind:CampaignPackKind}|undefined;if(!caption||caption.releaseId!==input.releaseId||caption.kind!=="caption")throw new Error("Select a caption from this release");if(caption.status!=="approved")throw new Error("Approve the caption before adding it to the publishing queue");if(input.mediaGenerationId){const media=this.getMediaGeneration(input.mediaGenerationId);if(!media||media.releaseId!==input.releaseId||media.status!=="approved")throw new Error("Select approved media from this release");}if(!["Instagram","Facebook","TikTok","SoundCloud","YouTube"].includes(input.platform))throw new Error("Invalid publishing platform");const id=randomUUID(),now=new Date().toISOString();this.database.prepare(`INSERT INTO publishing_queue(id,source_type,release_id,campaign_pack_item_id,media_generation_id,platform,caption,scheduled_at,status,created_at,updated_at) VALUES(?,'release',?,?,?,?,?,?,'draft',?,?)`).run(id,input.releaseId,input.campaignPackItemId,input.mediaGenerationId,input.platform,caption.content,input.scheduledAt||null,now,now);return this.getPublishingQueueItem(id)!;}
  enqueueEventCampaignItem(id:string):PublishingQueueItem{const item=this.listEventCampaignItems().find((entry)=>entry.id===id);if(!item)throw new Error("Event campaign item not found");if(item.status!=="APPROVED")throw new Error("Approve the event campaign item before adding it to the publishing queue");if(!item.scheduledAt)throw new Error("Schedule the event campaign item before adding it to the publishing queue");if(!item.content.trim())throw new Error("Event campaign content is required");if(!["Instagram","Facebook"].includes(item.platform))throw new Error(`${item.platform} is not supported by publishing yet`);if(item.publishingQueueId)throw new Error("This event campaign item is already in the publishing queue");const event=this.getEvent(item.eventId);if(!event)throw new Error("Event not found");const queueId=randomUUID(),now=new Date().toISOString();try{this.database.prepare(`INSERT INTO publishing_queue(id,source_type,event_id,event_campaign_item_id,platform,caption,scheduled_at,status,created_at,updated_at) VALUES(?,'event',?,?,?,?,?,'draft',?,?)`).run(queueId,event.id,item.id,item.platform,item.content,item.scheduledAt,now,now);}catch(error){if(error instanceof Error&&/UNIQUE constraint failed/.test(error.message))throw new Error("This event campaign item is already in the publishing queue");throw error;}return this.getPublishingQueueItem(queueId)!;}
  listPublishingQueue():PublishingQueueItem[]{const rows=this.database.prepare(`SELECT q.id,q.source_type AS sourceType,COALESCE(q.release_id,q.event_id) AS sourceId,COALESCE(r.title,e.name) AS sourceTitle,q.release_id AS releaseId,COALESCE(r.title,e.name) AS releaseTitle,q.campaign_pack_item_id AS campaignPackItemId,q.event_id AS eventId,e.name AS eventName,q.event_campaign_item_id AS eventCampaignItemId,q.platform,q.media_generation_id AS mediaGenerationId,q.caption,q.scheduled_at AS scheduledAt,q.status,q.error,q.exported_at AS exportedAt,q.remote_post_id AS remotePostId,q.published_at AS publishedAt,q.destination_id AS destinationId,m.media_type AS mediaType,m.provider AS mediaProvider,CASE WHEN q.release_id IS NULL THEN 0 ELSE EXISTS(SELECT 1 FROM soundcloud_tracks sc WHERE sc.release_id=q.release_id AND sc.content_type='bootleg') END AS rightsBlocked,se.id AS sourceScheduleEventId,COALESCE(ci.title,eci.title) AS sourceCampaignItemTitle,q.reviewed_by AS reviewedBy,q.reviewed_at AS reviewedAt,q.review_reason AS reviewReason,q.created_at AS createdAt,q.updated_at AS updatedAt FROM publishing_queue q LEFT JOIN releases r ON r.id=q.release_id LEFT JOIN events_v1 e ON e.id=q.event_id LEFT JOIN event_campaign_items_v1 eci ON eci.id=q.event_campaign_item_id LEFT JOIN media_generations m ON m.id=q.media_generation_id LEFT JOIN schedule_events se ON se.publishing_queue_id=q.id LEFT JOIN campaign_items ci ON ci.id=se.campaign_item_id ORDER BY CASE WHEN q.scheduled_at IS NULL THEN 1 ELSE 0 END,q.scheduled_at,q.created_at DESC`).all() as unknown as Array<Omit<PublishingQueueItem,"rightsBlocked">&{rightsBlocked:number}>;return rows.map((row)=>({...row,rightsBlocked:Boolean(row.rightsBlocked)}));}
  ...
  listContacts():ContactSummary[]{const rows=this.database.prepare(`SELECT c.id,c.name,c.contact_type AS contactType,c.relationship_status AS relationshipStatus,c.artist_id AS artistId,a.name AS artistName,c.release_id AS releaseId,r.title AS releaseTitle,c.organization,c.email,c.phone,c.website,c.social_handle AS socialHandle,c.preferred_channel AS preferredChannel,c.consent,c.notes,c.next_follow_up_at AS nextFollowUpAt,c.last_contact_at AS lastContactAt,c.created_at AS createdAt,c.updated_at AS updatedAt FROM contacts c LEFT JOIN artist_profiles a ON a.id=c.artist_id LEFT JOIN releases r ON r.id=c.release_id ORDER BY CASE c.relationship_status WHEN 'to-contact' THEN 0 WHEN 'conversation' THEN 1 WHEN 'new' THEN 2 ELSE 3 END,COALESCE(c.next_follow_up_at,'9999'),c.name`) .all() as unknown as Array<Omit<ContactSummary,"interactions"|"consent">&{consent:number}>;const interactions=this.database.prepare(`SELECT id,contact_id AS contactId,channel,direction,summary,occurred_at AS occurredAt,created_at AS createdAt FROM contact_interactions WHERE contact_id=? ORDER BY occurred_at DESC`);return rows.map((row)=>({...row,consent:Boolean(row.consent),interactions:interactions.all(row.id) as unknown as ContactInteraction[]}));}
  saveContact(input:UpsertContactInput):ContactSummary{const name=input.name.trim();if(!name)throw new Error("Contact name is required");if(input.releaseId){const release=this.listReleases().find((item)=>item.id===input.releaseId);if(!release)throw new Error("Release not found");if(input.artistId&&release.artistId!==input.artistId)throw new Error("Contact alias and release alias do not match");}const id=input.id??randomUUID(),now=new Date().toISOString(),nullable=(value:string)=>value.trim()||null;this.database.prepare(`INSERT INTO contacts (id,name,contact_type,relationship_status,artist_id,release_id,organization,email,phone,website,social_handle,preferred_channel,consent,notes,next_follow_up_at,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET name=excluded.name,contact_type=excluded.contact_type,relationship_status=excluded.relationship_status,artist_id=excluded.artist_id,release_id=excluded.release_id,organization=excluded.organization,email=excluded.email,phone=excluded.phone,website=excluded.website,social_handle=excluded.social_handle,preferred_channel=excluded.preferred_channel,consent=excluded.consent,notes=excluded.notes,next_follow_up_at=excluded.next_follow_up_at,updated_at=excluded.updated_at`).run(id,name,input.contactType,input.relationshipStatus,input.artistId,input.releaseId,nullable(input.organization),nullable(input.email),nullable(input.phone),nullable(input.website),nullable(input.socialHandle),input.preferredChannel,input.consent?1:0,input.notes.trim(),input.nextFollowUpAt,now,now);if(input.createFollowUpTask&&input.nextFollowUpAt){if(!input.releaseId)throw new Error("Select a release before creating a follow-up task");const link=this.database.prepare("SELECT project_id AS projectId FROM releases WHERE id=?").get(input.releaseId) as {projectId:string};const sourceKey=`contact-followup:${id}:${input.nextFollowUpAt}`;this.database.prepare(`INSERT OR IGNORE INTO tasks (id,project_id,release_id,title,status,priority,assignee,due_at,source_key,created_at,updated_at) VALUES (?,?,?,?,'todo','medium','human',?,?,?,?)`).run(randomUUID(),link.projectId,input.releaseId,`Follow up with ${name}`,input.nextFollowUpAt,sourceKey,now,now);}return this.listContacts().find((item)=>item.id===id)!;}
  deleteContact(id:string):void{const result=this.database.prepare("DELETE FROM contacts WHERE id=?").run(id);if(!result.changes)throw new Error("Contact not found");}
  addContactInteraction(input:AddContactInteractionInput):ContactSummary{const summary=input.summary.trim();if(!summary)throw new Error("Interaction summary is required");if(!this.listContacts().some((contact)=>contact.id===input.contactId))throw new Error("Contact not found");const now=new Date().toISOString();this.database.prepare(`INSERT INTO contact_interactions (id,contact_id,channel,direction,summary,occurred_at,created_at) VALUES (?,?,?,?,?,?,?)`).run(randomUUID(),input.contactId,input.channel,input.direction,summary,input.occurredAt||now,now);this.database.prepare("UPDATE contacts SET last_contact_at=?,relationship_status=CASE WHEN relationship_status IN ('new','to-contact') THEN 'contacted' ELSE relationship_status END,updated_at=? WHERE id=?").run(input.occurredAt||now,now,input.contactId);return this.listContacts().find((contact)=>contact.id===input.contactId)!;}
  ...
  listEvents():EventSummary[]{return (this.database.prepare("SELECT id FROM events_v1 ORDER BY event_date ASC,created_at ASC").all() as Array<{id:string}>).map(row=>this.getEvent(row.id)!).filter(Boolean);}
  getEvent(id:string):EventSummary|null{const row=this.database.prepare("SELECT id,kind,name,status,event_date AS date,doors_time AS doorsTime,start_time AS startTime,end_time AS endTime,timezone,venue,address,genre,description,ticket_url AS ticketUrl,event_url AS eventUrl,social_links_json AS socialLinks,organizer,created_at AS createdAt,updated_at AS updatedAt FROM events_v1 WHERE id=?").get(id) as (Omit<EventSummary,"lineup"|"assets"|"campaignProgress"|"socialLinks">&{socialLinks:string})|undefined;if(!row)return null;const lineup=this.database.prepare("SELECT id,event_id AS eventId,artist_name AS artistName,performance_start AS performanceStart,performance_end AS performanceEnd,sort_order AS sortOrder FROM event_lineup_v1 WHERE event_id=? ORDER BY sort_order,id").all(id) as unknown as EventLineupSlot[];const assets=this.database.prepare("SELECT id,event_id AS eventId,kind,file_path AS filePath,file_name AS fileName,mime_type AS mimeType,metadata_json AS metadata,created_at AS createdAt FROM event_assets_v1 WHERE event_id=? ORDER BY created_at").all(id) as unknown as Array<Omit<EventAsset,"metadata">&{metadata:string}>;const counts=this.database.prepare("SELECT COUNT(*) AS total,SUM(CASE WHEN status='APPROVED' THEN 1 ELSE 0 END) AS approved FROM event_campaign_items_v1 WHERE event_id=?").get(id) as {total:number;approved:number|null};return{...row,socialLinks:parseStringArray(row.socialLinks),lineup,assets:assets.map(a=>({...a,metadata:JSON.parse(a.metadata||"{}")})),campaignProgress:counts.total?Math.round((counts.approved??0)*100/counts.total):0};}
  saveEvent(input:UpsertEventInput):EventSummary{const name=input.name.trim(),date=input.date.trim(),timezone=input.timezone.trim(),venue=input.venue.trim();if(!name)throw new Error("Event name is required");if(!/^\d{4}-\d{2}-\d{2}$/.test(date)||Number.isNaN(new Date(`${date}T00:00:00Z`).getTime()))throw new Error("Invalid event date");normalizeScheduleTimezone(timezone);for(const value of [input.doorsTime,input.startTime,input.endTime].filter(Boolean) as string[]){if(!/^([01]\d|2[0-3]):[0-5]\d$/.test(value))throw new Error("Invalid event time");}for(const value of [input.ticketUrl,input.eventUrl,...(input.socialLinks??[])].filter(Boolean) as string[]){if(!/^https?:\/\//i.test(value))throw new Error("Event links must use http or https");}const now=new Date().toISOString(),id=input.id??randomUUID();this.database.exec("BEGIN IMMEDIATE");try{this.database.prepare("INSERT INTO events_v1(id,kind,name,status,event_date,doors_time,start_time,end_time,timezone,venue,address,genre,description,ticket_url,event_url,social_links_json,organizer,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET kind=excluded.kind,name=excluded.name,status=excluded.status,event_date=excluded.event_date,doors_time=excluded.doors_time,start_time=excluded.start_time,end_time=excluded.end_time,timezone=excluded.timezone,venue=excluded.venue,address=excluded.address,genre=excluded.genre,description=excluded.description,ticket_url=excluded.ticket_url,event_url=excluded.event_url,social_links_json=excluded.social_links_json,organizer=excluded.organizer,updated_at=excluded.updated_at").run(id,input.kind,name,input.status,date,input.doorsTime??null,input.startTime??null,input.endTime??null,timezone,venue,input.address?.trim()??"",input.genre?.trim()??"",input.description?.trim()??"",input.ticketUrl?.trim()||null,input.eventUrl?.trim()||null,JSON.stringify(input.socialLinks??[]),input.organizer?.trim()??"",now,now);this.database.prepare("DELETE FROM event_lineup_v1 WHERE event_id=?").run(id);const insert=this.database.prepare("INSERT INTO event_lineup_v1(id,event_id,artist_name,performance_start,performance_end,sort_order) VALUES(?,?,?,?,?,?)");(input.lineup??[]).forEach((slot,index)=>{if(!slot.artistName.trim())throw new Error("Lineup artist name is required");insert.run(slot.id??randomUUID(),id,slot.artistName.trim(),slot.performanceStart??null,slot.performanceEnd??null,slot.sortOrder??index);});this.database.exec("COMMIT");}catch(error){this.database.exec("ROLLBACK");throw error;}return this.getEvent(id)!;}
  deleteEvent(id:string):void{this.database.prepare("DELETE FROM events_v1 WHERE id=?").run(id);}
  attachEventAsset(input:AttachEventAssetInput):EventAsset{if(!this.getEvent(input.eventId))throw new Error("Event not found");const id=randomUUID(),createdAt=new Date().toISOString();this.database.prepare("INSERT INTO event_assets_v1(id,event_id,kind,file_path,file_name,mime_type,metadata_json,created_at) VALUES(?,?,?,?,?,?,?,?)").run(id,input.eventId,input.kind,input.filePath,input.fileName,input.mimeType,JSON.stringify(input.metadata??{}),createdAt);return{id,eventId:input.eventId,kind:input.kind,filePath:input.filePath,fileName:input.fileName,mimeType:input.mimeType,metadata:input.metadata??{},createdAt};}
  detachEventAsset(id:string):void{this.database.prepare("DELETE FROM event_assets_v1 WHERE id=?").run(id);}
  listEventCampaignItems(eventId?:string|null):EventCampaignItem[]{const rows=(eventId?this.database.prepare("SELECT eci.*,q.id AS publishing_queue_id FROM event_campaign_items_v1 eci LEFT JOIN publishing_queue q ON q.event_campaign_item_id=eci.id WHERE eci.event_id=? ORDER BY eci.scheduled_at,eci.created_at").all(eventId):this.database.prepare("SELECT eci.*,q.id AS publishing_queue_id FROM event_campaign_items_v1 eci LEFT JOIN publishing_queue q ON q.event_campaign_item_id=eci.id ORDER BY eci.scheduled_at,eci.created_at").all()) as Array<Record<string,unknown>>;return rows.map(row=>({id:String(row.id),eventId:String(row.event_id),title:String(row.title),contentType:row.content_type as EventCampaignItem["contentType"],language:row.language as ContentLanguage,platform:row.platform as CampaignChannel,tone:String(row.tone),content:String(row.content),scheduledAt:row.scheduled_at as string|null,status:row.status as EventCampaignStatus,publishingQueueId:row.publishing_queue_id as string|null,createdAt:String(row.created_at),updatedAt:String(row.updated_at)}));}
  saveEventCampaignItem(input:Omit<EventCampaignItem,"id"|"publishingQueueId"|"createdAt"|"updatedAt">):EventCampaignItem{if(!this.getEvent(input.eventId))throw new Error("Event not found");const id=randomUUID(),now=new Date().toISOString();this.database.prepare("INSERT INTO event_campaign_items_v1(id,event_id,title,content_type,language,platform,tone,content,scheduled_at,status,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)").run(id,input.eventId,input.title,input.contentType,input.language,input.platform,input.tone,input.content,input.scheduledAt,input.status,now,now);return this.listEventCampaignItems(input.eventId).find(x=>x.id===id)!;}
  updateEventCampaignItem(id:string,input:{content?:string;scheduledAt?:string|null;status?:EventCampaignStatus}):EventCampaignItem{const current=this.listEventCampaignItems().find(x=>x.id===id);if(!current)throw new Error("Event campaign item not found");const scheduledAt=input.scheduledAt===undefined?current.scheduledAt:input.scheduledAt?(new Date(input.scheduledAt).toISOString()):null;this.database.prepare("UPDATE event_campaign_items_v1 SET content=?,scheduled_at=?,status=?,updated_at=? WHERE id=?").run(input.content??current.content,scheduledAt,input.status??current.status,new Date().toISOString(),id);return this.listEventCampaignItems(current.eventId).find(x=>x.id===id)!;}
