...
,
  ...
  {
    version: 30,
    name: "add_kling_cli_provider",
    sql: `
      CREATE TABLE media_generations_v3 (
        id TEXT PRIMARY KEY,
        release_id TEXT NOT NULL REFERENCES releases(id) ON DELETE CASCADE,
        campaign_pack_item_id TEXT NOT NULL REFERENCES campaign_pack_items(id) ON DELETE CASCADE,
        provider TEXT NOT NULL CHECK(provider IN ('openai','kling','kling-cli','comfyui')),
        media_type TEXT NOT NULL CHECK(media_type IN ('image','video')),
        prompt TEXT NOT NULL,
        status TEXT NOT NULL DEFAULT 'queued' CHECK(status IN ('queued','generating','ready','failed','approved','rejected')),
        provider_task_id TEXT,
        local_path TEXT,
        mime_type TEXT,
        error TEXT,
      ...
      INSERT INTO media_generations_v3 SELECT * FROM media_generations;
      DROP TABLE media_generations;
