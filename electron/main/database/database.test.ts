...
test("creates a clean database, applies migrations and persists a release", () => {
  ...
  try {
    ...
    const legacyQueueId = "legacy-queue-item";
    const databaseHandle = database as unknown as { database: { prepare(sql: string): { run(...values: unknown[]): unknown } } };
    databaseHandle.database.prepare("INSERT INTO publishing_queue(id,release_id,campaign_pack_item_id,media_generation_id,platform,caption,scheduled_at,status,created_at,updated_at) VALUES(?,?,?,?,?,?,?,'draft',?,?)").run(legacyQueueId,created.id,pack[0]!.id,null,"Instagram","Campaign caption","2026-09-10T18:00:00.000Z","2026-09-01T00:00:00.000Z","2026-09-01T00:00:00.000Z");
    const queued=database.getPublishingQueueItem(legacyQueueId)!;
    assert.equal(queued.sourceScheduleEventId,null);assert.equal(queued.status,"draft");assert.equal(database.updatePublishingQueueStatus(queued.id,"approved").status,"approved");assert.equal(database.updatePublishingQueueStatus(queued.id,"scheduled").status,"scheduled");assert.equal(database.markPublishingSucceeded(queued.id,"facebook:page-1","remote-post-1").remotePostId,"remote-post-1");
