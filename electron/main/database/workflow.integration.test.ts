...
test("vertical release workflow survives restart and preserves publishing state", () => {
  ...
  try {
    ...
    const release = database.createReleaseDraft({
      artistId: "the-arkadiusz",
      title: "Phase 2C Vertical Slice",
      primaryGenre: "Psytrance",
      story: "Integration-test release workflow.",
    ...
    const audio = database.attachAsset({
      releaseId,
      kind: "audio",
      filePath: path.join(directory, "master.wav"),
      fileName: "master.wav",
      mimeType: "audio/wav",
      sizeBytes: 1000,
      modifiedAt: null,
    ...
    database.saveAudioAnalysis({
      ...
      analyzedAt: "2026-08-30T20:00:00.000Z",
      note: null
    });
    database.attachAsset({
      releaseId,
      kind: "cover",
      filePath: path.join(directory, "cover.png"),
      fileName: "cover.png",
      mimeType: "image/png",
      sizeBytes: 2000,
      modifiedAt: null,
    ...
    assert.equal(caption.status, "approved");
    assert.equal(imagePrompt.status, "approved");

    const media = database.createMediaGeneration(imagePrompt, "comfyui", "image");
    const readyMedia = database.updateMediaGeneration(media.id, {
      status: "ready",
      localPath: path.join(directory, "generated-cover.png"),
      mimeType: "image/png"
    });
    assert.equal(readyMedia.status, "ready");

    const queueItem = database.createPublishingQueueItem({
      releaseId,
      campaignPackItemId: caption.id,
      mediaGenerationId: readyMedia.id,
      platform: "Instagram",
      scheduledAt: "2026-09-17T18:00:00.000Z"
    });
    queueId = queueItem.id;
    assert.equal(queueItem.status, "draft");
