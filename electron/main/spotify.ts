...
export class SpotifyClient {
  ...
  async fetchArtistReleases(artistId: string): Promise<unknown[]> { const token = await this.token(); let next: string | null = `https://api.spotify.com/v1/artists/${encodeURIComponent(artistId)}/albums?include_groups=album,single,appears_on,compilation&limit=50`; const result: unknown[] = []; while (next) { const page = await this.fetchJson(next, token) as { items?: unknown[]; next?: string | null }; result.push(...(page.items ?? [])); next = page.next ?? null; } return result; }
  ...
  private async fetchJson(url: string, token: string): Promise<unknown> { const response = await fetch(url, { headers: { Authorization: `Bearer ${token}` }, signal: AbortSignal.timeout(30_000) }); if (!response.ok) throw new Error(`Spotify API returned HTTP ${response.status}`); return response.json(); }
