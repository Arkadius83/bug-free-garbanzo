...
export class LocalServicesManager {
  ...
  async startConfigured():Promise<void>{const settings=await this.readSettings();if(!settings.autoStart)return;await this.start("ollama").catch(()=>undefined);}
